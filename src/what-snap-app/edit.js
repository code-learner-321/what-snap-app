		...
		<div {...blockProps}>
			<InspectorControls>
				<TabPanel
					...
					tabs={[
						{ name: 'settings', title: __('Settings', 'what-snap-app') },
					]}
				>
					{(tab) => (
						<div className={`tab-panel-${tab.name}`}>
							{tab.name === 'settings' && (
								<PanelBody title={__('Pre-filled Messages', 'what-snap-app')}>
									<TextControl
										label={__('WhatsApp Message', 'what-snap-app')}
										value={message}
			...
			<div className="what-snap-app-button-preview">
				<a href="#" class="whatsapp-btn" rel="noopener" onClick={(e) => e.preventDefault()}>
					<svg class="whatsapp-icon" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
						<g clip-path={`url(#clip0_${clientId})`}>
